...
#[pyfunction]
fn bezier_surf_eval_dp(i: usize, j: usize, n: usize, m: usize, dim: usize, u: f64, v: f64) -> PyResult<Vec<f64>> {
...
#[pyfunction]
fn bezier_surf_dsdu_dp(i: usize, j: usize, n: usize, m: usize, dim: usize, u: f64, v: f64) -> PyResult<Vec<f64>> {
    let float_n = n as f64;
...
#[pyfunction]
fn bezier_surf_dsdv_dp(i: usize, j: usize, n: usize, m: usize, dim: usize, u: f64, v: f64) -> PyResult<Vec<f64>> {
    let float_m = m as f64;
...
#[pyfunction]
fn bezier_surf_d2sdu2_dp(i: usize, j: usize, n: usize, m: usize, dim: usize, u: f64, v: f64) -> PyResult<Vec<f64>> {
    let float_n = n as f64;
...
#[pyfunction]
fn bezier_surf_d2sdv2_dp(i: usize, j: usize, n: usize, m: usize, dim: usize, u: f64, v: f64) -> PyResult<Vec<f64>> {
    let float_m = m as f64;
...
#[pyfunction]
fn bezier_surf_dsdu_dp_iso_v(i: usize, j: usize, n: usize, m: usize, dim: usize, nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let float_n = n as f64;
    let mut dp_mat: Vec<Vec<f64>> = vec![vec![0.0; dim]; nu];
    for u_idx in 0..nu {
        let u = (u_idx as f64) * 1.0 / (nu as f64 - 1.0);
...
#[pyfunction]
fn bezier_surf_dsdv_dp_iso_u(i: usize, j: usize, n: usize, m: usize, dim: usize, u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let float_m = m as f64;
    let mut dp_mat: Vec<Vec<f64>> = vec![vec![0.0; dim]; nv];
    for v_idx in 0..nv {
        let v = (v_idx as f64) * 1.0 / (nv as f64 - 1.0);
...
#[pyfunction]
fn bezier_surf_dsdv_dp_iso_v(i: usize, j: usize, n: usize, m: usize, dim: usize, nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let float_m = m as f64;
    let mut dp_mat: Vec<Vec<f64>> = vec![vec![0.0; dim]; nu];
    for u_idx in 0..nu {
        let u = (u_idx as f64) * 1.0 / (nu as f64 - 1.0);
...
#[pyfunction]
fn rational_bezier_curve_eval_dp(w: Vec<f64>, i: usize, n: usize, dim: usize, t: f64) -> PyResult<Vec<f64>> {
    let mut w_sum: f64 = 0.0;
    for ii in 0..n+1 {
        w_sum += w[ii] * bernstein_poly_rust(n, ii, t);
    }
    let dp_val: f64 = w[i] * bernstein_poly_rust(n, i, t) / w_sum;
...
#[pyfunction]
fn rational_bezier_curve_dcdt_dp(w: Vec<f64>, i: usize, n: usize, dim: usize, t: f64) -> PyResult<Vec<f64>> {
    let float_n = n as f64;
    ...
    for ii in 0..n+1 {  // Use a separate index nomenclature from the one specified as an argument
        let b_poly_t = bernstein_poly_rust(n, ii, t);
        let b_poly_dt = bernstein_poly_rust(n - 1, ii - 1, t) - bernstein_poly_rust(n - 1, ii, t);
        w_sum_0 += w[ii] * b_poly_t;
        w_sum_1 += w[ii] * b_poly_dt;
        if ii == i {
            dp_0 = w[ii] * b_poly_t;
            dp_1 = w[ii] * b_poly_dt;
        }
...
#[pyfunction]
fn rational_bezier_curve_d2cdt2_dp(w: Vec<f64>, i: usize, n: usize, dim: usize, t: f64) -> PyResult<Vec<f64>> {
    let float_n = n as f64;
    let mut dp_0: f64 = 0.0;
    let mut dp_1: f64 = 0.0;
    ...
    for ii in 0..n+1 {  // Use a separate index nomenclature from the one specified as an argument
        let b_poly_t = bernstein_poly_rust(n, ii, t);
        let b_poly_dt = bernstein_poly_rust(n - 1, ii - 1, t) - bernstein_poly_rust(n - 1, ii, t);
        let b_poly_d2t = bernstein_poly_rust(n - 2, ii - 2, t) - 2.0 * bernstein_poly_rust(n - 2, ii - 1, t) + bernstein_poly_rust(n - 2, ii, t);
        w_sum_0 += w[ii] * b_poly_t;
        w_sum_1 += w[ii] * b_poly_dt;
        w_sum_2 += w[ii] * b_poly_d2t;
        if ii == i {
            dp_0 = w[ii] * b_poly_t;
            dp_1 = w[ii] * b_poly_dt;
            dp_2 = w[ii] * b_poly_d2t;
        }
    }
    let dp_val: f64 = (
...
#[pyfunction]
fn rational_bezier_curve_eval_dp_grid(w: Vec<f64>, i: usize, n: usize, dim: usize, nt: usize) -> PyResult<Vec<Vec<f64>>> {
    let mut dp_mat: Vec<Vec<f64>> = vec![vec![0.0; dim]; nt];
    for t_idx in 0..nt {
        let t = (t_idx as f64) * 1.0 / (nt as f64 - 1.0);
        let mut w_sum: f64 = 0.0;
        for ii in 0..n+1 {
...
#[pyfunction]
fn rational_bezier_curve_dcdt_dp_grid(w: Vec<f64>, i: usize, n: usize, dim: usize, nt: usize) -> PyResult<Vec<Vec<f64>>> {
    let float_n = n as f64;
    let mut dp_mat: Vec<Vec<f64>> = vec![vec![0.0; dim]; nt];
    for t_idx in 0..nt {
        let t = (t_idx as f64) * 1.0 / (nt as f64 - 1.0);
...
#[pyfunction]
fn rational_bezier_curve_d2cdt2_dp_grid(w: Vec<f64>, i: usize, n: usize, dim: usize, nt: usize) -> PyResult<Vec<Vec<f64>>> {
    let float_n = n as f64;
    let mut dp_mat: Vec<Vec<f64>> = vec![vec![0.0; dim]; nt];
    for t_idx in 0..nt {
        let t = (t_idx as f64) * 1.0 / (nt as f64 - 1.0);
...
#[pyfunction]
fn rational_bezier_curve_eval_dp_tvec(w: Vec<f64>, i: usize, n: usize, dim: usize, t: Vec<f64>) -> PyResult<Vec<Vec<f64>>> {
    let nt = t.len();
    let mut dp_mat: Vec<Vec<f64>> = vec![vec![0.0; dim]; nt];
    for t_idx in 0..nt {
        let mut w_sum: f64 = 0.0;
        for ii in 0..n+1 {
            w_sum += w[ii] * bernstein_poly_rust(n, ii, t[t_idx]);
        }
        let dp_val: f64 = w[i] * bernstein_poly_rust(n, i, t[t_idx]) / w_sum;
        for j in 0..dim {
            dp_mat[t_idx][j] = dp_val;
        }
    }
...
#[pyfunction]
fn rational_bezier_curve_dcdt_dp_tvec(w: Vec<f64>, i: usize, n: usize, dim: usize, t: Vec<f64>) -> PyResult<Vec<Vec<f64>>> {
    let float_n = n as f64;
    let nt = t.len();
    let mut dp_mat: Vec<Vec<f64>> = vec![vec![0.0; dim]; nt];
    for t_idx in 0..nt {
        ...
        let mut w_sum_1: f64 = 0.0;
        for ii in 0..n+1 {  // Use a separate index nomenclature from the one specified as an argument
            let b_poly_t = bernstein_poly_rust(n, ii, t[t_idx]);
            let b_poly_dt = bernstein_poly_rust(n - 1, ii - 1, t[t_idx]) - bernstein_poly_rust(n - 1, ii, t[t_idx]);
            w_sum_0 += w[ii] * b_poly_t;
            w_sum_1 += w[ii] * b_poly_dt;
            if ii == i {
                dp_0 = w[ii] * b_poly_t;
...
#[pyfunction]
fn rational_bezier_curve_d2cdt2_dp_tvec(w: Vec<f64>, i: usize, n: usize, dim: usize, t: Vec<f64>) -> PyResult<Vec<Vec<f64>>> {
    let float_n = n as f64;
    let nt = t.len();
    let mut dp_mat: Vec<Vec<f64>> = vec![vec![0.0; dim]; nt];
    for t_idx in 0..nt {
        ...
        for ii in 0..n+1 {  // Use a separate index nomenclature from the one specified as an argument
            let b_poly_t = bernstein_poly_rust(n, ii, t[t_idx]);
            let b_poly_dt = bernstein_poly_rust(n - 1, ii - 1, t[t_idx]) - bernstein_poly_rust(n - 1, ii, t[t_idx]);
            let b_poly_d2t = bernstein_poly_rust(n - 2, ii - 2, t[t_idx]) - 2.0 * bernstein_poly_rust(n - 2, ii - 1, t[t_idx]) + bernstein_poly_rust(n - 2, ii, t[t_idx]);
            w_sum_0 += w[ii] * b_poly_t;
            w_sum_1 += w[ii] * b_poly_dt;
            w_sum_2 += w[ii] * b_poly_d2t;
            if ii == i {
    ...
    for v_idx in 0..nv {
        ...
        for i in 0..n+1 {
            ...
            for j in 0..m+1 {
                ...
                w_sum += w[i][j] * b_poly_prod;
                for k in 0..dim {
                    evaluated_points[v_idx][k] += p[i][j][k] * w[i][j] * b_poly_prod;
                }
            }
        }
        for k in 0..dim {
            evaluated_points[v_idx][k] /= w_sum;
        }
    }
    Ok(evaluated_points)
}
...
#[pyfunction]
fn rational_bezier_surf_eval_iso_v(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ...
    for u_idx in 0..nu {
        ...
        for i in 0..n+1 {
            ...
            for j in 0..m+1 {
                ...
                w_sum += w[i][j] * b_poly_prod;
                for k in 0..dim {
                    evaluated_points[u_idx][k] += p[i][j][k] * w[i][j] * b_poly_prod;
                }
            }
        }
        for k in 0..dim {
            evaluated_points[u_idx][k] /= w_sum;
        }
    }
    Ok(evaluated_points)
}
...
#[pyfunction]
fn rational_bezier_surf_dsdu_iso_u(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>, 
    u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let float_n = n as f64;
    ...
    for v_idx in 0..nv {
        ...
        for k in 0..dim {
            evaluated_derivs[v_idx][k] = float_n * (sum_1[k] * w_sum_0 - sum_0[k] * w_sum_1) / (w_sum_0 * w_sum_0);
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn rational_bezier_surf_dsdu_iso_v(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>, 
    nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let float_n = n as f64;
    ...
    for u_idx in 0..nu {
        ...
        for k in 0..dim {
            evaluated_derivs[u_idx][k] = float_n * (sum_1[k] * w_sum_0 - sum_0[k] * w_sum_1) / (w_sum_0 * w_sum_0);
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn rational_bezier_surf_dsdv_iso_u(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>, 
    u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let float_m = m as f64;
    ...
    for v_idx in 0..nv {
        ...
        for k in 0..dim {
            evaluated_derivs[v_idx][k] = float_m * (sum_1[k] * w_sum_0 - sum_0[k] * w_sum_1) / (w_sum_0 * w_sum_0);
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn rational_bezier_surf_dsdv_iso_v(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>, 
    nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let float_m = m as f64;
    ...
    for u_idx in 0..nu {
        ...
        for k in 0..dim {
            evaluated_derivs[u_idx][k] = float_m * (sum_1[k] * w_sum_0 - sum_0[k] * w_sum_1) / (w_sum_0 * w_sum_0);
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn rational_bezier_surf_d2sdu2_iso_u(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>, 
    u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let float_n = n as f64;
...
#[pyfunction]
fn rational_bezier_surf_d2sdu2_iso_v(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>, 
    nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let float_n = n as f64;
...
#[pyfunction]
fn rational_bezier_surf_d2sdv2_iso_u(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let float_m = m as f64;
...
#[pyfunction]
fn rational_bezier_surf_d2sdv2_iso_v(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let float_m = m as f64;
...
#[pyfunction]
fn rational_bezier_surf_dsdu_grid(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>, 
    nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let float_n = n as f64;
    ...
    for u_idx in 0..nu {
        ...
        for v_idx in 0..nv {
            ...
            for k in 0..dim {
                evaluated_derivs[u_idx][v_idx][k] = float_n * (sum_1[k] * w_sum_0 - sum_0[k] * w_sum_1) / (w_sum_0 * w_sum_0);
            }
        }
    }
    Ok(evaluated_derivs)
...
#[pyfunction]
fn rational_bezier_surf_dsdv_grid(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>, 
    nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let float_m = m as f64;
    ...
    for u_idx in 0..nu {
        ...
        for v_idx in 0..nv {
            ...
            for k in 0..dim {
                evaluated_derivs[u_idx][v_idx][k] = float_m * (sum_1[k] * w_sum_0 - sum_0[k] * w_sum_1) / (w_sum_0 * w_sum_0);
            }
        }
    }
    Ok(evaluated_derivs)
...
#[pyfunction]
fn rational_bezier_surf_d2sdu2_grid(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>, 
    nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let float_n = n as f64;
    ...
    for u_idx in 0..nu {
        ...
        for v_idx in 0..nv {
            ...
            for k in 0..dim {
                evaluated_derivs[u_idx][v_idx][k] = (
                    float_n * (float_n - 1.0) * sum_2[k] * w_sum_0 * w_sum_0 - 
                    float_n * (float_n - 1.0) * sum_0[k] * w_sum_0 * w_sum_2 -
                    2.0 * float_n * float_n * sum_1[k] * w_sum_0 * w_sum_1 +
                    2.0 * float_n * float_n * sum_0[k] * w_sum_1 * w_sum_1
...
#[pyfunction]
fn rational_bezier_surf_d2sdv2_grid(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let float_m = m as f64;
...
#[pyfunction]
fn rational_bezier_surf_eval_uvvecs(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
...
#[pyfunction]
fn rational_bezier_surf_dsdu_uvvecs(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>, 
    u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let nu = u.len();
    ...
    for u_idx in 0..nu {
        for v_idx in 0..nv {
            ...
            for i in 0..n+1 {
                let b_poly_u = bernstein_poly_rust(n, i, u[u_idx]);
                let b_poly_du = bernstein_poly_rust(n - 1, i - 1, u[u_idx]) - bernstein_poly_rust(n - 1, i, u[u_idx]);
                for j in 0..m+1 {
                    let b_poly_v = bernstein_poly_rust(m, j, v[v_idx]);
                    w_sum_0 += w[i][j] * b_poly_u * b_poly_v;
                    w_sum_1 += w[i][j] * b_poly_du * b_poly_v;
...
#[pyfunction]
fn rational_bezier_surf_dsdv_uvvecs(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>, 
    u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let nu = u.len();
    ...
    for u_idx in 0..nu {
        for v_idx in 0..nv {
            ...
            for j in 0..m+1 {
                let b_poly_v = bernstein_poly_rust(m, j, v[v_idx]);
                let b_poly_dv = bernstein_poly_rust(m - 1, j - 1, v[v_idx]) - bernstein_poly_rust(m - 1, j, v[v_idx]);
                for i in 0..n+1 {
                    let b_poly_u = bernstein_poly_rust(n, i, u[u_idx]);
                    w_sum_0 += w[i][j] * b_poly_v * b_poly_u;
                    w_sum_1 += w[i][j] * b_poly_dv * b_poly_u;
...
#[pyfunction]
fn rational_bezier_surf_d2sdu2_uvvecs(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>, 
    u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let nu = u.len();
    ...
    for u_idx in 0..nu {
        for v_idx in 0..nv {
            ...
            for i in 0..n+1 {
                let b_poly_u = bernstein_poly_rust(n, i, u[u_idx]);
                let b_poly_du = bernstein_poly_rust(n - 1, i - 1, u[u_idx]) - bernstein_poly_rust(n - 1, i, u[u_idx]);
                let b_poly_d2u = bernstein_poly_rust(n - 2, i - 2, u[u_idx]) - 2.0 * bernstein_poly_rust(n - 2, i - 1, u[u_idx]) + bernstein_poly_rust(n - 2, i, u[u_idx]);
                for j in 0..m+1 {
                    let b_poly_v = bernstein_poly_rust(m, j, v[v_idx]);
                    w_sum_0 += w[i][j] * b_poly_u * b_poly_v;
                    w_sum_1 += w[i][j] * b_poly_du * b_poly_v;
...
#[pyfunction]
fn rational_bezier_surf_d2sdv2_uvvecs(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let nu = u.len();
    ...
    for u_idx in 0..nu {
        for v_idx in 0..nv {
            ...
            for j in 0..m+1 {
                let b_poly_v = bernstein_poly_rust(m, j, v[v_idx]);
                let b_poly_dv = bernstein_poly_rust(m - 1, j - 1, v[v_idx]) - bernstein_poly_rust(m - 1, j, v[v_idx]);
                let b_poly_d2v = bernstein_poly_rust(m - 2, j - 2, v[v_idx]) - 2.0 * bernstein_poly_rust(m - 2, j - 1, v[v_idx]) + bernstein_poly_rust(m - 2, j, v[v_idx]);
                for i in 0..n+1 {
                    let b_poly_u = bernstein_poly_rust(n, i, u[u_idx]);
                    w_sum_0 += w[i][j] * b_poly_v * b_poly_u;
                    w_sum_1 += w[i][j] * b_poly_dv * b_poly_u;
...
#[pyfunction]
fn bspline_surf_eval_dp(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: f64, v: f64) -> PyResult<Vec<f64>> {
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_point: Vec<f64> = vec![0.0; dim];
    let bspline_basis_u = cox_de_boor(&ku, &possible_span_indices_u, q, i, u);
    ...
    let bspline_basis_prod = bspline_basis_u * bspline_basis_v;
    for k in 0..dim {
        evaluated_point[k] = bspline_basis_prod;
    }
    Ok(evaluated_point)
}

#[pyfunction]
fn bspline_surf_dsdu_dp(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: f64, v: f64) -> PyResult<Vec<f64>> {
    let float_q = q as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_deriv: Vec<f64> = vec![0.0; dim];
    ...
    let bspline_basis_prod = bspline_basis_du * bspline_basis_v;
    for k in 0..dim {
        evaluated_deriv[k] = float_q * bspline_basis_prod;
    }
    Ok(evaluated_deriv)
}

#[pyfunction]
fn bspline_surf_dsdv_dp(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: f64, v: f64) -> PyResult<Vec<f64>> {
    let float_r = r as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_deriv: Vec<f64> = vec![0.0; dim];
    ...
    let bspline_basis_prod = bspline_basis_u * bspline_basis_dv;
    for k in 0..dim {
        evaluated_deriv[k] = float_r * bspline_basis_prod;
    }
    Ok(evaluated_deriv)
}

#[pyfunction]
fn bspline_surf_d2sdu2_dp(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: f64, v: f64) -> PyResult<Vec<f64>> {
    let float_q = q as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_deriv: Vec<f64> = vec![0.0; dim];
    ...
    let bspline_basis_prod = bspline_basis_d2u * bspline_basis_v;
    for k in 0..dim {
        evaluated_deriv[k] = float_q * (float_q - 1.0) * bspline_basis_prod;
    }
    Ok(evaluated_deriv)
}

#[pyfunction]
fn bspline_surf_d2sdv2_dp(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: f64, v: f64) -> PyResult<Vec<f64>> {
    let float_r = r as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_deriv: Vec<f64> = vec![0.0; dim];
    ...
    let bspline_basis_prod = bspline_basis_u * bspline_basis_d2v;
    for k in 0..dim {
        evaluated_deriv[k] = float_r * (float_r - 1.0) * bspline_basis_prod;
    }
    Ok(evaluated_deriv)
}
...
#[pyfunction]
fn bspline_surf_eval_dp_iso_u(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_points: Vec<Vec<f64>> = vec![vec![0.0; dim]; nv];
    for v_idx in 0..nv {
        ...
        let bspline_basis_prod = bspline_basis_u * bspline_basis_v;
        for k in 0..dim {
            evaluated_points[v_idx][k] = bspline_basis_prod;
        }
    }
    Ok(evaluated_points)
}
...
#[pyfunction]
fn bspline_surf_eval_dp_iso_v(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize,
    nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_points: Vec<Vec<f64>> = vec![vec![0.0; dim]; nu];
    for u_idx in 0..nu {
        ...
        let bspline_basis_prod = bspline_basis_u * bspline_basis_v;
        for k in 0..dim {
            evaluated_points[u_idx][k] = bspline_basis_prod;
        }
    }
    Ok(evaluated_points)
}
...
#[pyfunction]
fn bspline_surf_dsdu_dp_iso_u(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let float_q = q as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_derivs: Vec<Vec<f64>> = vec![vec![0.0; dim]; nv];
    for v_idx in 0..nv {
        ...
        let bspline_basis_prod = bspline_basis_du * bspline_basis_v;
        for k in 0..dim {
            evaluated_derivs[v_idx][k] = float_q * bspline_basis_prod;
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn bspline_surf_dsdu_dp_iso_v(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let float_q = q as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_derivs: Vec<Vec<f64>> = vec![vec![0.0; dim]; nu];
    for u_idx in 0..nu {
        ...
        let bspline_basis_prod = bspline_basis_du * bspline_basis_v;
        for k in 0..dim {
            evaluated_derivs[u_idx][k] = float_q * bspline_basis_prod;
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn bspline_surf_dsdv_dp_iso_u(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let float_r = r as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_derivs: Vec<Vec<f64>> = vec![vec![0.0; dim]; nv];
    for v_idx in 0..nv {
        ...
        let bspline_basis_prod = bspline_basis_u * bspline_basis_dv;
        for k in 0..dim {
            evaluated_derivs[v_idx][k] = float_r * bspline_basis_prod;
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn bspline_surf_dsdv_dp_iso_v(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let float_r = r as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_derivs: Vec<Vec<f64>> = vec![vec![0.0; dim]; nu];
    for u_idx in 0..nu {
        ...
        let bspline_basis_prod = bspline_basis_u * bspline_basis_dv;
        for k in 0..dim {
            evaluated_derivs[u_idx][k] = float_r * bspline_basis_prod;
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn bspline_surf_d2sdu2_dp_iso_u(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let float_q = q as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_derivs: Vec<Vec<f64>> = vec![vec![0.0; dim]; nv];
    ...
    for v_idx in 0..nv {
        ...
        let bspline_basis_prod = bspline_basis_d2u * bspline_basis_v;
        for k in 0..dim {
            evaluated_derivs[v_idx][k] = float_q * (float_q - 1.0) * bspline_basis_prod;
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn bspline_surf_d2sdu2_dp_iso_v(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let float_q = q as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_derivs: Vec<Vec<f64>> = vec![vec![0.0; dim]; nu];
    ...
    for u_idx in 0..nu {
        ...
        let bspline_basis_prod = bspline_basis_d2u * bspline_basis_v;
        for k in 0..dim {
            evaluated_derivs[u_idx][k] = float_q * (float_q - 1.0) * bspline_basis_prod;
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn bspline_surf_d2sdv2_dp_iso_u(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let float_r = r as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_derivs: Vec<Vec<f64>> = vec![vec![0.0; dim]; nv];
    ...
    for v_idx in 0..nv {
        ...
        let bspline_basis_prod = bspline_basis_u * bspline_basis_d2v;
        for k in 0..dim {
            evaluated_derivs[v_idx][k] = float_r * (float_r - 1.0) * bspline_basis_prod;
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn bspline_surf_d2sdv2_dp_iso_v(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let float_r = r as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_derivs: Vec<Vec<f64>> = vec![vec![0.0; dim]; nu];
    ...
    for u_idx in 0..nu {
        ...
        let bspline_basis_prod = bspline_basis_u * bspline_basis_d2v;
        for k in 0..dim {
            evaluated_derivs[u_idx][k] = float_r * (float_r - 1.0) * bspline_basis_prod;
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn bspline_surf_eval_dp_grid(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_points: Vec<Vec<Vec<f64>>> = vec![vec![vec![0.0; dim]; nv]; nu];
    for u_idx in 0..nu {
        ...
        for v_idx in 0..nv {
            ...
            let bspline_basis_prod = bspline_basis_u * bspline_basis_v;
            for k in 0..dim {
                evaluated_points[u_idx][v_idx][k] = bspline_basis_prod;
            }
        }
    }
    Ok(evaluated_points)
...
#[pyfunction]
fn bspline_surf_dsdu_dp_grid(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let float_q = q as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_derivs: Vec<Vec<Vec<f64>>> = vec![vec![vec![0.0; dim]; nv]; nu];
    for u_idx in 0..nu {
        ...
        for v_idx in 0..nv {
            ...
            let bspline_basis_prod = bspline_basis_du * bspline_basis_v;
            for k in 0..dim {
                evaluated_derivs[u_idx][v_idx][k] = float_q * bspline_basis_prod;
            }
        }
    }
    Ok(evaluated_derivs)
...
#[pyfunction]
fn bspline_surf_dsdv_dp_grid(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let float_r = r as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_derivs: Vec<Vec<Vec<f64>>> = vec![vec![vec![0.0; dim]; nv]; nu];
    for u_idx in 0..nu {
        ...
        for v_idx in 0..nv {
            ...
            let bspline_basis_prod = bspline_basis_u * bspline_basis_dv;
            for k in 0..dim {
                evaluated_derivs[u_idx][v_idx][k] = float_r * bspline_basis_prod;
            }
        }
    }
    Ok(evaluated_derivs)
...
#[pyfunction]
fn bspline_surf_d2sdu2_dp_grid(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let float_q = q as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_derivs: Vec<Vec<Vec<f64>>> = vec![vec![vec![0.0; dim]; nv]; nu];
    ...
    for u_idx in 0..nu {
        ...
        for v_idx in 0..nv {
            ...
            let bspline_basis_prod = bspline_basis_d2u * bspline_basis_v;
            for k in 0..dim {
                evaluated_derivs[u_idx][v_idx][k] = float_q * (float_q - 1.0) * bspline_basis_prod;
            }
        }
    }
    Ok(evaluated_derivs)
...
#[pyfunction]
fn bspline_surf_d2sdv2_dp_grid(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let float_r = r as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
    let mut evaluated_derivs: Vec<Vec<Vec<f64>>> = vec![vec![vec![0.0; dim]; nv]; nu];
    ...
    for u_idx in 0..nu {
        ...
        for v_idx in 0..nv {
            ...
            let bspline_basis_prod = bspline_basis_u * bspline_basis_d2v;
            for k in 0..dim {
                evaluated_derivs[u_idx][v_idx][k] = float_r * (float_r - 1.0) * bspline_basis_prod;
            }
        }
    }
    Ok(evaluated_derivs)
...
#[pyfunction]
fn bspline_surf_eval_dp_uvvecs(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let nu = u.len();
    let nv = v.len();
    let possible_span_indices_u = get_possible_span_indices(&ku);
    let possible_span_indices_v = get_possible_span_indices(&kv);
...
#[pyfunction]
fn bspline_surf_dsdu_dp_uvvecs(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let nu = u.len();
    let nv = v.len();
    let float_q = q as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
...
#[pyfunction]
fn bspline_surf_dsdv_dp_uvvecs(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let nu = u.len();
    let nv = v.len();
    let float_r = r as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
...
#[pyfunction]
fn bspline_surf_d2sdu2_dp_uvvecs(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let nu = u.len();
    let nv = v.len();
    let float_q = q as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
...
#[pyfunction]
fn bspline_surf_d2sdv2_dp_uvvecs(ku: Vec<f64>, kv: Vec<f64>, i: usize, j: usize, q: usize, r: usize, dim: usize, u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let nu = u.len();
    let nv = v.len();
    let float_r = r as f64;
    let possible_span_indices_u = get_possible_span_indices(&ku);
...
#[pyfunction]
fn nurbs_surf_eval_iso_u(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
    let num_knots_v = kv.len();  // Number of knots in the v-direction
    ...
    for v_idx in 0..nv {
        ...
        for i in 0..n+1 {
            ...
            for j in 0..m+1 {
                ...
                w_sum += w[i][j] * bspline_basis_prod;
                for k in 0..dim {
                    evaluated_points[v_idx][k] += p[i][j][k] * w[i][j] * bspline_basis_prod;
                }
            }
        }
        for k in 0..dim {
...
#[pyfunction]
fn nurbs_surf_eval_iso_v(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
    let num_knots_v = kv.len();  // Number of knots in the v-direction
    ...
    for u_idx in 0..nu {
        ...
        for i in 0..n+1 {
            ...
            for j in 0..m+1 {
                ...
                w_sum += w[i][j] * bspline_basis_prod;
                for k in 0..dim {
                    evaluated_points[u_idx][k] += p[i][j][k] * w[i][j] * bspline_basis_prod;
                }
            }
        }
        for k in 0..dim {
...
#[pyfunction]
fn nurbs_surf_dsdu_iso_u(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
    ...
    for v_idx in 0..nv {
        ...
        for k in 0..dim {
            evaluated_derivs[v_idx][k] = float_q * (sum_1[k] * w_sum_0 - sum_0[k] * w_sum_1) / (w_sum_0 * w_sum_0);
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn nurbs_surf_dsdu_iso_v(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
    ...
    for u_idx in 0..nu {
        ...
        for k in 0..dim {
            evaluated_derivs[u_idx][k] = float_q * (sum_1[k] * w_sum_0 - sum_0[k] * w_sum_1) / (w_sum_0 * w_sum_0);
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn nurbs_surf_dsdv_iso_u(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
    ...
    for v_idx in 0..nv {
        ...
        for k in 0..dim {
            evaluated_derivs[v_idx][k] = float_r * (sum_1[k] * w_sum_0 - sum_0[k] * w_sum_1) / (w_sum_0 * w_sum_0);
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn nurbs_surf_dsdv_iso_v(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
    ...
    for u_idx in 0..nu {
        ...
        for k in 0..dim {
            evaluated_derivs[u_idx][k] = float_r * (sum_1[k] * w_sum_0 - sum_0[k] * w_sum_1) / (w_sum_0 * w_sum_0);
        }
    }
    Ok(evaluated_derivs)
}
...
#[pyfunction]
fn nurbs_surf_d2sdu2_iso_u(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
...
#[pyfunction]
fn nurbs_surf_d2sdu2_iso_v(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
...
#[pyfunction]
fn nurbs_surf_d2sdv2_iso_u(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, u: f64, nv: usize) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
...
#[pyfunction]
fn nurbs_surf_d2sdv2_iso_v(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, nu: usize, v: f64) -> PyResult<Vec<Vec<f64>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
...
#[pyfunction]
fn nurbs_surf_dsdu_grid(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
    ...
    for u_idx in 0..nu {
        ...
        for v_idx in 0..nv {
            ...
            for k in 0..dim {
                evaluated_derivs[u_idx][v_idx][k] = float_q * (sum_1[k] * w_sum_0 - sum_0[k] * w_sum_1) / (w_sum_0 * w_sum_0);
            }
        }
    }
    Ok(evaluated_derivs)
...
#[pyfunction]
fn nurbs_surf_dsdv_grid(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
    ...
    for u_idx in 0..nu {
        ...
        for v_idx in 0..nv {
            ...
            for k in 0..dim {
                evaluated_derivs[u_idx][v_idx][k] = float_r * (sum_1[k] * w_sum_0 - sum_0[k] * w_sum_1) / (w_sum_0 * w_sum_0);
            }
        }
    }
    Ok(evaluated_derivs)
...
#[pyfunction]
fn nurbs_surf_d2sdu2_grid(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
...
#[pyfunction]
fn nurbs_surf_d2sdv2_grid(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, nu: usize, nv: usize) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let num_knots_u = ku.len();  // Number of knots in the u-direction
...
#[pyfunction]
fn nurbs_surf_eval_uvvecs(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let nu = u.len();
    let nv = v.len();
...
#[pyfunction]
fn nurbs_surf_dsdu_uvvecs(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let nu = u.len();
    ...
    for u_idx in 0..nu {
        for v_idx in 0..nv {
            ...
            for i in 0..n+1 {
                ...
                let span_a: f64 = ku[i + q] - ku[i];
                let span_b: f64 = ku[i + q + 1] - ku[i + 1];
                let bspline_basis_0 = cox_de_boor(&ku, &possible_span_indices_u, q, i, u[u_idx]);
                if span_a != 0.0 {
                    ka = 1.0 / span_a;
                }
                if span_b != 0.0 {
                ...
                let bspline_basis_1 = ka * cox_de_boor(&ku, &possible_span_indices_u, q - 1, i, u[u_idx]) - kb * cox_de_boor(&ku, &possible_span_indices_u, q - 1, i + 1, u[u_idx]);
                for j in 0..m+1 {
                    let bspline_basis_v = cox_de_boor(&kv, &possible_span_indices_v, r, j, v[v_idx]);
                    w_sum_0 += w[i][j] * bspline_basis_0 * bspline_basis_v;
                    w_sum_1 += w[i][j] * bspline_basis_1 * bspline_basis_v;
...
#[pyfunction]
fn nurbs_surf_dsdv_uvvecs(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let nu = u.len();
    ...
    for u_idx in 0..nu {
        for v_idx in 0..nv {
            ...
            for j in 0..m+1 {
                ...
                let span_a: f64 = kv[j + r] - kv[j];
                let span_b: f64 = kv[j + r + 1] - kv[j + 1];
                let bspline_basis_0 = cox_de_boor(&kv, &possible_span_indices_v, r, j, v[v_idx]);
                if span_a != 0.0 {
                    ka = 1.0 / span_a;
                }
                if span_b != 0.0 {
                ...
                let bspline_basis_1 = ka * cox_de_boor(&kv, &possible_span_indices_v, r - 1, j, v[v_idx]) - kb * cox_de_boor(&kv, &possible_span_indices_v, r - 1, j + 1, v[v_idx]);
                for i in 0..n+1 {
                    let bspline_basis_u = cox_de_boor(&ku, &possible_span_indices_u, q, i, u[u_idx]);
                    w_sum_0 += w[i][j] * bspline_basis_0 * bspline_basis_u;
                    w_sum_1 += w[i][j] * bspline_basis_1 * bspline_basis_u;
...
#[pyfunction]
fn nurbs_surf_d2sdu2_uvvecs(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let nu = u.len();
    ...
    for u_idx in 0..nu {
        for v_idx in 0..nv {
            ...
            for i in 0..n+1 {
                ...
                let bspline_basis_1 = ka * cox_de_boor(&ku, &possible_span_indices_u, q - 1, i, u[u_idx]) - kb * cox_de_boor(&ku, &possible_span_indices_u, q - 1, i + 1, u[u_idx]);
                let bspline_basis_d = kd * cox_de_boor(&ku, &possible_span_indices_u, q - 2, i + 1, u[u_idx]);
                let bspline_basis_2 = ka * (kc * cox_de_boor(&ku, &possible_span_indices_u, q - 2, i, u[u_idx]) - bspline_basis_d) - kb * (bspline_basis_d - ke * cox_de_boor(&ku, &possible_span_indices_u, q - 2, i + 2, u[u_idx]));
                for j in 0..m+1 {
                    let bspline_basis_v = cox_de_boor(&kv, &possible_span_indices_v, r, j, v[v_idx]);
                    w_sum_0 += w[i][j] * bspline_basis_0 * bspline_basis_v;
                    w_sum_1 += w[i][j] * bspline_basis_1 * bspline_basis_v;
...
#[pyfunction]
fn nurbs_surf_d2sdv2_uvvecs(p: Vec<Vec<Vec<f64>>>, w: Vec<Vec<f64>>,
    ku: Vec<f64>, kv: Vec<f64>, u: Vec<f64>, v: Vec<f64>) -> PyResult<Vec<Vec<Vec<f64>>>> {
    let n = p.len() - 1;  // Number of control points in the u-direction minus 1
    let m = p[0].len() - 1;  // Number of control points in the v-direction minus 1
    let nu = u.len();
    ...
    for u_idx in 0..nu {
        for v_idx in 0..nv {
            ...
            for j in 0..m+1 {
                ...
                let bspline_basis_1 = ka * cox_de_boor(&kv, &possible_span_indices_v, r - 1, j, v[v_idx]) - kb * cox_de_boor(&kv, &possible_span_indices_v, r - 1, j + 1, v[v_idx]);
                let bspline_basis_d = kd * cox_de_boor(&kv, &possible_span_indices_v, r - 2, j + 1, v[v_idx]);
                let bspline_basis_2 = ka * (kc * cox_de_boor(&kv, &possible_span_indices_v, r - 2, j, v[v_idx]) - bspline_basis_d) - kb * (bspline_basis_d - ke * cox_de_boor(&kv, &possible_span_indices_v, r - 2, j + 2, v[v_idx]));
                for i in 0..n+1 {
                    let bspline_basis_u = cox_de_boor(&ku, &possible_span_indices_u, q, i, u[u_idx]);
                    w_sum_0 += w[i][j] * bspline_basis_0 * bspline_basis_u;
                    w_sum_1 += w[i][j] * bspline_basis_1 * bspline_basis_u;
...
fn rust_nurbs(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(bezier_surf_eval_dp, m)?)?;
    m.add_function(wrap_pyfunction!(bezier_surf_dsdu_dp, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(rational_bezier_curve_eval_dp, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_dcdt, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_dcdt_dp, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_d2cdt2, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_d2cdt2_dp, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_eval_grid, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_eval_dp_grid, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_dcdt_grid, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_dcdt_dp_grid, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_d2cdt2_grid, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_d2cdt2_dp_grid, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_eval_tvec, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_eval_dp_tvec, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_dcdt_tvec, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_dcdt_dp_tvec, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_d2cdt2_tvec, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_curve_d2cdt2_dp_tvec, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(rational_bezier_surf_dsdu_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_dsdv_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_dsdv_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_d2sdu2_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_d2sdu2_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_d2sdv2_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_d2sdv2_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_eval_grid, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_dsdu_grid, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_dsdv_grid, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_d2sdu2_grid, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_d2sdv2_grid, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_eval_uvvecs, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_dsdu_uvvecs, m)?)?;
    m.add_function(wrap_pyfunction!(rational_bezier_surf_dsdv_uvvecs, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(bspline_surf_eval_dp, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_dsdu_dp, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_dsdv_dp, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_d2sdu2_dp, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_d2sdv2_dp, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(bspline_surf_eval_dp_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_eval_dp_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_dsdu_dp_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_dsdu_dp_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_dsdv_dp_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_dsdv_dp_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_d2sdu2_dp_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_d2sdu2_dp_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_d2sdv2_dp_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_d2sdv2_dp_iso_v, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(bspline_surf_eval_dp_grid, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_dsdu_dp_grid, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_dsdv_dp_grid, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_d2sdu2_dp_grid, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_d2sdv2_dp_grid, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(bspline_surf_eval_dp_uvvecs, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_dsdu_dp_uvvecs, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_dsdv_dp_uvvecs, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_d2sdu2_dp_uvvecs, m)?)?;
    m.add_function(wrap_pyfunction!(bspline_surf_d2sdv2_dp_uvvecs, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(nurbs_surf_eval_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_eval_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_dsdu_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_dsdu_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_dsdv_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_dsdv_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_d2sdu2_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_d2sdu2_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_d2sdv2_iso_u, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_d2sdv2_iso_v, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_eval_grid, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_dsdu_grid, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_dsdv_grid, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_d2sdu2_grid, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_d2sdv2_grid, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_eval_uvvecs, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_dsdu_uvvecs, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_dsdv_uvvecs, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_d2sdu2_uvvecs, m)?)?;
    m.add_function(wrap_pyfunction!(nurbs_surf_d2sdv2_uvvecs, m)?)?;
    Ok(())
}
